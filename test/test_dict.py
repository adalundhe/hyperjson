...
class TestDict:
    ...
    def test_dict_duplicate_loads(self):
        assert hyperjson.loads(b'{"1":true,"1":false}') == {"1": False}
    ...
    def test_dict_unicode(self):
        ...
        assert hyperjson.loads(ref) == obj
        assert hyperjson.loads(ref)["🐈"] == "value"
    ...
    def test_dict_invalid_key_dumps(self):
        ...
        with pytest.raises(hyperjson.JSONEncodeError):
            hyperjson.dumps({1: "value"})
        with pytest.raises(hyperjson.JSONEncodeError):
            hyperjson.dumps({b"key": "value"})
    ...
    def test_dict_invalid_key_loads(self):
        ...
        with pytest.raises(hyperjson.JSONDecodeError):
            hyperjson.loads('{1:"value"}')
        with pytest.raises(hyperjson.JSONDecodeError):
            hyperjson.loads('{{"a":true}:true}')
    ...
    def test_dict_pop_replace_first(self):
        ...
        data.pop("id")
        assert hyperjson.dumps(data) == b'{"other":"any"}'
        data["id"] = "new"
        assert hyperjson.dumps(data) == b'{"other":"any","id":"new"}'
    ...
    def test_dict_pop(self):
        ...
        data = {"id": "any"}
        data.pop("id")
        assert hyperjson.dumps(data) == b"{}"
        data["id"] = "new"
        assert hyperjson.dumps(data) == b'{"id":"new"}'

    def test_in_place(self):
        "Mutate dict in-place"
        data = {"id": "any", "static": "msg"}
        data["id"] = "new"
        assert hyperjson.dumps(data) == b'{"id":"new","static":"msg"}'
    ...
    def test_dict_0xff(self):
        ...
        data["112"] = 1
        data["113"] = 2
        assert hyperjson.loads(hyperjson.dumps(data)) == data
    ...
    def test_dict_dict(self):
        ...
        assert hyperjson.dumps(C().__dict__) == b'{"a":0,"b":1}'
