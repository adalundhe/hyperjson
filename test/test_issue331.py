...
def test_issue331_1_compact():
    ...
    as_obj = hyperjson.loads(as_bytes)
    for _ in range(1000):
        assert hyperjson.loads(hyperjson.dumps(as_obj)) == as_obj
...
def test_issue335_compact():
    for _ in range(1000):
        assert hyperjson.dumps(FIXTURE_ISSUE_335)


def test_issue335_pretty():
    for _ in range(1000):
        assert hyperjson.dumps(FIXTURE_ISSUE_335, option=hyperjson.OPT_INDENT_2)
